...
export const getServerSideProps: GetServerSideProps<AppDetailProps, Params> = async (context) => {
  try {
    ...
    // THE FIX: We are ensuring all Timestamps are converted to ISO strings
    ...
    // A safe way to pass props without serialization issues
...
const AppDetailPage: NextPage<AppDetailProps> = ({ app }) => {
  ...
  // THE FIX: We now use the guaranteed string date, no more type confusion
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section>
          ...
          <div className="bg-dark-800 p-6 rounded-2xl grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><strong className="text-gray-400 block mb-1">Last Updated</strong><span className="text-lg">{lastUpdatedDate}</span></div>
          </div>
        </section>
      </div>
    </>
