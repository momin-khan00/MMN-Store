...
export const getServerSideProps: GetServerSideProps<AppDetailProps, Params> = async (context) => {
  try {
    ...
    if (!docSnap.exists() || docSnap.data().status !== 'approved') {
      return { notFound: true }; // Return 404 if app doesn't exist or is not approved
    }
    
    // We need to convert Firestore Timestamp to a serializable format (string)
...
const AppDetailPage: NextPage<AppDetailProps> = ({ app }) => {
  ...
  return (
    <>
      <Head>
        <title>{app.name} - MMN Store</title>
      </Head>
        ...
        <section>
          ...
          <div className="bg-dark-800 p-4 rounded-2xl">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><strong className="text-gray-400">Updated On:</strong> {new Date(app.createdAt.toString()).toLocaleDateString()}</div>
            </div>
          </div>
        </section>
      </div>
